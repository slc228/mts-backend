...
public class DataRepresentation {
	...
	public static class DataCore {
		...
		public void build_features(String text, int windowsSize, int n) {
	        ...
	        for (int sentence_id = 0; sentence_id < this.sentences_str.size(); sentence_id++) {
	            ...
	            for (int pos_in_sentence = 0; pos_in_sentence < sentence.size(); pos_in_sentence++) {
	            	String word = sentence.get(pos_in_sentence);
	            	// 判断当前词是否全部由符号构成
	            	int cExclude = 0;
	                ...
	                if (cExclude == word.length()) {
	                    ...
	                    String tag = this.getTag(word, pos_in_sentence);
	    ...
	    public List<List<String>> get_sentences_str(String text){
			List<List<String>> sentences_str = new ArrayList<>();
			String[] sentences = text.split("[。！？]");
			for (String sentence : sentences) {
				List<com.hankcs.hanlp.seg.common.Term> words = HanLP.segment(sentence);
				List<String> stringWords = new ArrayList<>();
				for(com.hankcs.hanlp.seg.common.Term t : words){
					stringWords.add(t.word);
				}
				sentences_str.add(stringWords);
			}
